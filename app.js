...
const scriptUrl = 'https://script.google.com/macros/s/AKfycbxdGcA8tjlu68qb6PJhiOj3Gv8ymS_SecwYMvA8b4DNUDoKycbfgYJ3O9pCf8xnnyuO/exec';

// Track new user
function trackNewUser(name) {
  fetch(`${scriptUrl}?action=newuser&name=${encodeURIComponent(name)}`)
    .then(response => response.text())
    .then(() => updateCounts())
    .catch(err => console.error('Error tracking user:', err));
}

// Update all counts from the sheet
function updateCounts() {
  fetch(`${scriptUrl}?action=getcounts`)
    .then(response => response.json())
    .then(data => {
      document.getElementById('totalLikes').innerHTML = data.likes;
      document.getElementById('totalDislikes').innerHTML = data.dislikes;
...
document.getElementById('like-Btn').addEventListener('click', () => {
  fetch(`${scriptUrl}?action=like`)
    .then(response => response.text())
    .then(() => updateCounts())
    .catch(err => console.error('Error recording like:', err));
});

document.getElementById('unlike-Btn').addEventListener('click', () => {
  fetch(`${scriptUrl}?action=dislike`)
    .then(response => response.text())
    .then(() => updateCounts())
